            ...
            if ((i === 0 && j < startingDay) || date > daysInMonth) {
                ...
                if (userShifts.length > 0) {
                    userShifts.forEach(shift => {
                        ...
                        // Check for advanced shift
                        if (shift.is_advanced_shift) {
                            taskDiv.classList.add('task-advanced');
                        }
                        
                        // Get shift type name
                        const shiftTypeName = shiftTypeNames[shift.shift_type_id] || 'Unknown';
                        
                        // Create header with shift type name
                        const header = document.createElement('div');
                        const timeRange = shift.start_time && shift.end_time ? `${formatTime(shift.start_time)} - ${formatTime(shift.end_time)}` : 'No Time';
                        header.innerHTML = `<strong>${shiftTypeName}</strong><br><small>${timeRange}</small>`;
                        ...
                        clientsWrapper.style.gap = '4px';
                        
                        let clientIds = [];
                        if (shift.clientIds) {
                            clientIds = Array.isArray(shift.clientIds) ? shift.clientIds : [shift.clientIds];
                        } else if (shift.client_id) {
                        ...
                        clientIds.forEach(id => {
                            ...
                            clientNameText += (clientNameText ? ', ' : '') + clientName;
                        });
                        
                        // Show client names instead of count
                        if (clientNameText) {
                            const clientNameSpan = document.createElement('span');
                            clientNameSpan.textContent = clientNameText;
                            clientNameSpan.style.fontSize = '10px';
                            clientNameSpan.style.color = '#666';
                            clientNameSpan.style.marginLeft = '4px';
                            clientsWrapper.appendChild(clientNameSpan);
                        }
                        taskDiv.appendChild(clientsWrapper);
                        
                        // Check for next day shift (overnight)
                        // Logic same as schedular.blade.php: checks shift_finishes_next_day OR end_time < start_time
                        const isOvernightShift = !!shift.shift_finishes_next_day ||
                            (!!shift.start_time && !!shift.end_time && shift.end_time < shift.start_time);
                        
                        // Add NEXT DAY text after shift type name for overnight shifts
                        if (isOvernightShift) {
                            const nextDayLabel = document.createElement('span');
                            nextDayLabel.textContent = ' - NEXT DAY';
                            nextDayLabel.style.display = 'block';
                            nextDayLabel.style.fontSize = '9px';
                            nextDayLabel.style.fontWeight = 'bold';
                            nextDayLabel.style.color = '#0ea5e9';
                            header.appendChild(nextDayLabel);
                        }
                        if (isOvernightShift) {
                            taskDiv.classList.add('task-overnight');
                        }
                        
                        // Add status icon based on shift status
                        if (shift.is_approved === 1) {
                            const statusIcon = document.createElement('i');
                            statusIcon.className = 'fas fa-check-circle status-icon status-icon-booked';
                            statusIcon.title = 'Approved';
                            taskDiv.appendChild(statusIcon);
                        } else if (shift.status === 'invoiced') {
                            const statusIcon = document.createElement('i');
                            statusIcon.className = 'fas fa-file-invoice-dollar status-icon status-icon-invoiced';
                            statusIcon.title = 'Invoiced';
                            taskDiv.appendChild(statusIcon);
                        } else if (shift.is_cancelled) {
                            const statusIcon = document.createElement('i');
                            statusIcon.className = 'fas fa-times-circle status-icon status-icon-cancelled';
                            statusIcon.title = 'Cancelled';
                            taskDiv.appendChild(statusIcon);
                        } else {
                            const statusIcon = document.createElement('i');
                            statusIcon.className = 'fas fa-clock status-icon status-icon-pending';
                            statusIcon.title = 'Pending';
                            taskDiv.appendChild(statusIcon);
                        }
                        
                        // Handle cancelled - strikethrough
